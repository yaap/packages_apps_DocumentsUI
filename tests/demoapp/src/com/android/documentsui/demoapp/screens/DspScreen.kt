...
/**
 * A screen that demonstrates using the Downloads Storage Provider (DSP).
 *
 * This screen provides options to interact with files in the Downloads directory using
 * `com.android.providers.downloads.documents`.
 *
 * @param modifier Modifier to be applied to the layout.
 */
@Composable
fun DspScreen(modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp)) {
        NoteSection(stringResource(R.string.note_dsp))
        ...
        Button(
            onClick = {
                val uri =
                    DocumentsContract.buildDocumentUri(
                        DocumentsContract.DOWNLOADS_PROVIDER_AUTHORITY,
                        "downloads",
                    )
                val intent =
            ...
            Text(stringResource(R.string.btn_pick_dsp_downloads))
        }
